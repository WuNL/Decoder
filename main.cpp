...
//    std::cout<<req.target().data()<<std::endl;
    ...
    if (! req.target().compare("/InitDecoder/"))
    {
        std::cout << "receive request: InitDecoder" << std::endl;
        std::cout << "InitDecoder body content:" << req.body() << std::endl;
        initParams p;
        if (jsonParser(req.body(), p))
            return send(bad_request("Illegal Init Param"));

        initParamsRet ret = g_conductor.initDecoder(p);
        ...
        if (ret.success)
        {
            ...
            char strbuf[200];
            sprintf(strbuf, R"({"decoder_name": "%s", "raw_shm_id": "%d", "codeced_shm_id": "%d"})",
                    ret.decoder_name.c_str(), ret.yuv_shmID, ret.out_shmID);
    ...
    if (! req.target().compare("/DestroyDecoder/"))
    {
        std::cout << "receive request: DestroyDecoder" << std::endl;
        std::cout << "DestroyDecoder body content:" << req.body() << std::endl;
        destroyParams p;
        jsonParserDestroy(req.body(), p);
        if (p.success)
        {
            //destroy success
            if (g_conductor.destroyDecoder(p))
            {
                http::response<http::string_body> res{http::status::ok, req.version()};
                res.set(http::field::server, BOOST_BEAST_VERSION_STRING);
                res.set(http::field::content_type, "text/html");
                ...
                return send(std::move(res));
                std::cout << "after DestroyDecoder, the vector size is :" << g_conductor.getSize() << std::endl;
            }
                //destroy fail
            else
            {
